...
//  Tinder
//
//  Created by Evandro Rodrigo Minamoto on 19/11/21.
//
...
class MatchVC: UIViewController, UITextFieldDelegate {
    ...
    let fotoImageView: UIImageView = .fotoTmageView(named: "pessoa-1")
    
    let likeImageView: UIImageView = .fotoTmageView(named: "icone-like")
    ...
    let voltarButton: UIButton = {
        let voltar = UIButton()
        voltar.setTitle("Voltar para o Tinder", for: .normal)
        voltar.setTitleColor(.white, for: .normal)
        voltar.titleLabel?.font = .systemFont(ofSize: 16)
        return voltar
    }()
